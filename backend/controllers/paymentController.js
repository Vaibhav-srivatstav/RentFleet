...
console.log("AMOUNT RECEIVED:", amount);
        ...
        try {
            ...
            return res.status(500).json({
                success: false,
                message: 'Payment not completed', error: err.message
            })
        }
        let session;
        try {
...
export const confirmPayment = async (req, res) => {
    try {
        ...
        if (!session_id) return res.status(400).json({ success: false, message: 'Session_id require' });
        ...
        if (!order) return res.status(404).json({
            success: false,
            message: 'Booking not fount for this session',
            session
        })
        return res.json({
            session: true,
