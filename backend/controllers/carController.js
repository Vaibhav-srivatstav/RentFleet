...
export const createCar = async (req, res, next) => {
    try {
        const {
            make, model, dailyRate, category, description,year, color, seats, transmission, fuelType, mileage, status
        } = req.body;
    ...
    try {
        ...
        if (req.file) {
            ...
            if (!req.body.image && car.image) {
                const oldPath = path.join(process.cwd(), 'uplolads', car.image);
                fs.unlink(oldPath, (err) => {
                    if (err) console.warn('Failed to delete old image: ', err);
                })
                car.image = '';
