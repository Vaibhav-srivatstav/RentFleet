...
const Panel = ({ title, icon, children }) => (
    <div className={BookingPageStyles.panel}>
        <h3 className={BookingPageStyles.panelTitle}>{icon}
...
const StatusIndicator = ({ status, isEditing, newStatus, onStatusChange }) => {
    return isEditing ? (
        <select value={newStatus} onChange={onStatusChange} className='bg-gray-800/50 text-sm  px-2 py-1 rounded  focus:outline-none focus:ring-1 focus:ring-gray-500'>{Object.keys(statusConfig).filter((k) =>
            k !== 'default').map((opt) => (
...
const BookingCardHeader = ({ booking, onToggleDetails, isExpanded }) => (
    <div className={BookingPageStyles.bookingCardHeader}>
        ...
        <div>
            <div className={BookingPageStyles.bookingCustomer}>{booking.customer || ""}
            </div>
            <div className={BookingPageStyles.bookingEmail}>{booking.email || ""}
            </div>
        </div>
...
const BookingCardActions = ({ isEditing, onEditStatus, onSaveStatus, onCancelEdit, onToggleDetails, isExpanded }) => (
    <div className={BookingPageStyles.bookingActions}>
        <div className=' items-center text-gray-400 hidden md:flex'>
            <FaChevronDown className={`transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`} />
            <span className='ml-2 text-sm'>
                {isExpanded ? 'Hide Details' : 'Show Details'}
            </span>
        ...
        <div className='flex  space-x-3 ml-auto'>
            {isEditing ? (
                <>
                    <button onClick={onSaveStatus} className={BookingPageStyles.bookingActionButton('green')
                    }>Save</button>

                    <button onClick={onCancelEdit} className={BookingPageStyles.bookingActionButton('gray')
                    }>Cancel</button>
                </>
            ) : (
...
const BookingCard = ({
    booking, isExpanded, isEditing, newStatus,onEditStatus, onToggleDetails, onStatusChange, onSaveStatus, onCancelEdit
}) => (
    <div className={BookingPageStyles.bookingCard}>
        <div className='p-5 cursor-pointer' onClick={onToggleDetails}>
            <div className='flex flex-col md:flex-row justify-between items-start md:items-center'>
                ...
                <BookingCardInfo booking={booking} isEditing={isEditing} newStatus={newStatus} onStatusChange={onStatusChange} />
            </div>

            <BookingCardActions isEditing={isEditing} onEditStatus={onEditStatus} onSaveStatus={onSaveStatus} onCancelStatus={onCancelEdit} onToggleDetails={onToggleDetails} isExpanded={isExpanded} />
        </div>
...
const NoBookingView = ({ onResetFilter }) => (
    <div className={BookingPageStyles.noBookingsContainer}>
        ...
        <p className={BookingPageStyles.noBookingsText}>Try adjusting your search or filter criteria.</p>
...
const PageHeader = () => (
    <div className={BookingPageStyles.headerContainer}>
        <div className={BookingPageStyles.headerDivider}>
            <div className={BookingPageStyles.headerDividerLine}></div></div>
            <h1 className={BookingPageStyles.title}>
                <span className={BookingPageStyles.titleGradient}>Booking DashBoard</span>
            </h1>
            <p className={BookingPageStyles.subtitle}>Manage all booking with detail information and status update</p>
    ...
    return (
        <div className={BookingPageStyles.pageContainer}>
            ...
            <SearchFilterBar
                ...
                statusFilter={statusFilter} onStatusChange={(e) => setStatusFilter(e.target.value)} totalBookings={bookings.length}
            />

            <div className='space-y-4'>{filterBookings.map((booking)=>(
                <BookingCard key={booking.id} booking={booking} isExpanded={expandedBooking === booking.id}
                isEditing={editingStatus === booking.id} newStatus={newStatus} onToggleDetails={() => handleToggleDetails(booking.id)} onEditStatus={(e)=>{
                    e.stopPropagation();
            ...
            {filterBookings.length ===0 && (
                <NoBookingView onResetFilter={handleResetFilters}/>
            )}
            </div>
        </div>
    )
