...
const AddCar = () => {
    ...
    const renderInputField = (field) => {
        return (
            <div key={field.name}>
                ...
                <input required={field.required} name={field.name} value={data[field.name]} onChange={handleChange} type={field.type || 'text'}
                    className={field.prefix ? AddCarPageStyles.inputWithPrefix : AddCarPageStyles.input} placeholder={field.placeholder} min={field.min} max={field.max} {...field.props}
                />
            </div>
        );
    };
    ...
    const renderSelectField = (field) => (
        <div key={field.name}>
            <label className={AddCarPageStyles.label}>{field.name}</label>
            <select required={field.required} name={field.name} value={data[field.name]} onChange={handleChange} className={AddCarPageStyles.select}>{field.options.map(option => (
        ...
        <div className={AddCarPageStyles.pageContainer}>
            ...
            <div className={AddCarPageStyles.headerContainer}>
                <div className={AddCarPageStyles.headerDivider}>
                    <div className={AddCarPageStyles.headerDividerLine}>
                    </div>
                </div>
                <h1 className={AddCarPageStyles.title}><span className={AddCarPageStyles.titleGradient}>Add Your Car</span></h1>
                <p className={AddCarPageStyles.subtitle}>Share your vehicle with the world and start earning today.</p>
            </div>
                                ...
                                if (field.type === "input") {
                                    return (
                                        <div key={field.config.name}>
                                            <label className={AddCarPageStyles.label}>{field.config.label}</label>
                                            <div className='relative'>{field.config.prefix}
                                                <input required={field.config.required} name={field.config.name} value={data[field.config.name]} onChange={handleChange} type={field.config.type || 'text'}
                                                    className={`${AddCarPageStyles.input} ${field.config.props?.className || ''}`} placeholder={field.config.placeholder} min={field.config.min} max={field.config.max} {...field.config.props}
                                                />
                                            </div>
                                        </div>
                                    )
                        ...
                        <div className={AddCarPageStyles.formColumn}>
                            <div className={AddCarPageStyles.formGridInner}>
                                    ...
                                    <div className={AddCarPageStyles.imageUploadContainer}>
                                        <label className={AddCarPageStyles.imageUploadLabel} >
                                            {data.imagePreview ? (
                                                ...
                                                (<div className={AddCarPageStyles.imageUploadPlaceholder}> <svg
                                                ...
                                                >
                                                    ...
                                                    <p className={AddCarPageStyles.imageUploadText}><span className={AddCarPageStyles.imageUploadTextSemibold}>click to Upload Image</span>
                                                        <br />
                                                        or drag and drop</p>
                                                    <p className={AddCarPageStyles.imageUploadSubText}>PNG, JPG upto 5mb</p>
                                                </div>)}
                                ...
                                <div>
                                    ...
                                    <textarea required name="description" id="description" value={data.description} onChange={handleChange} rows='4' className={AddCarPageStyles.textarea} placeholder='Describe features, conditions, special details...' />
                                </div>
                            </div>
                        </div>

                        <div className='mt-12 flex justify-center'>
                            <button type='submit' className={AddCarPageStyles.submitButton}><span className={AddCarPageStyles.buttonText}>list your car</span>
