...
//SUB - COMPONENTS
const StartCard = ({ title, value, icon: Icon, className = '' }) => {
  return(
  <div className={`${styles.gradientOrange} ${styles.rounded2xl} ${styles.statCard} ${styles.borderGray}${styles.borderHoverOrange}${className}`}>
    <div className='flex items-starts justify-between'>
      <div >
        <h3 className={`${styles.textGray} text-sm font-medium md-2`}>{title}</h3>
        <p className='text-2xl font-bold text-white'>{value}</p>
      </div>
      <div className='p-3 rounded-lg bg-gray-800/30'>
        <Icon className={`${styles.textOrange} text-xl`} /></div>
    </div>
  </div>
  );
};
...
const CarCard = ({ car, onEdit, onDelete }) => {
  const getStatusStyle = (status) => {
    const styles = {
      ...
      rented: "bg-gray-900/30 text-yellow-400",
    ...
    <div className={`${styles.gradientGray} ${styles.rounded2xl} ${styles.carCard} ${styles.borderGray}${styles.borderHoverOrange}`}>
      <div className='relative'>
        ...
        <div className='absolute top-4 right-4'>
          <span className={`${styles.statusBadge} ${getStatusStyle(car.status)}`}>{car.status.charAt(0).toUpperCase() + car.status.slice(1)}
          </span>
        </div>
      </div>
      <div className='p-5'>
        <div className='flex justify-between items-start md-4'>
          <div>
            <h3 className='text-xl font-bold text-gray-600'>{car.make} {car.model}</h3>
            <p className={styles.textGray}>{car.year}</p>
          </div>
          <div className='text-2xl font-bold text-gray-500'>
            ₹{car.dailyRate}
            <span className='text-sm text-gray-900 font-normal'>/day</span>
          </div>
        </div>
        <div className='grid grid-cols-2 gap-4 mb-5'>
          ...
          <div className='flex justify-between border-t border-gray-800 pt-4'>
            <button onClick={() => onEdit(car)} className={`flex items-center ${styles.textOrange} hover:text-gray-800 transition-colors`}>
              <FaEdit className='mr-1' />Edit
            </button>
            <button onClick={() => onDelete(car._id ?? car.id)} className={`flex items-center ${styles.textRed} hover:text-red-800 transition-colors`}>
...
const EditModal = ({ car, onClose, onSubmit, onChange }) => {
  ...
  const handleInputChange = (e) => {
    ...
    onChange({
      ...car, [name]: ["year", "dailyRate", "mileage", "seats"].includes(name) ? value === '' ? "" : Number(value) : value,
    });
  };
            ...
            {car.image && (
              <div className="flex justify-center">
                <img
                  ...
                  alt="preview"
                  className="h-40 object-contain rounded-md border border-gray-800/30"
                />
              </div>
            )}
...
const NoCarsVeiw = ({ onResetFilter }) => {
  return(
  <div className={`${styles.gradientGray} ${styles.noCarsContainer}`}>
    <div className='mx-auto w-24 h-24 rounded-full bg-gradient-to-br from-gray-900/30 to.gray-800/90 flex items-center justify-center mb-6'>
      <div className='bg-gardient-to-br from-gray-700 to-gray-600 w-16 h-16 flex rounded-full justify-center item-center'>
        <FaCar className='h-8 w-8 text-white ' />
      </div>
    </div>
    <h3 className='mt-4 text-xl font-medium text-black'>No car found</h3>
  ...
  >
    ...
    <div className="relative">
      <select
        ...
        className={`${styles.inputField} focus:outline-none focus:ring-2 focus:ring-gary-500`}
      >
        {categories.map((c) => (
      ...
      <div className="absolute left-1 top-4 text-gray-500">
        <FaFilter />
      </div>
    </div>
  </div>
...
const ManageCar = () => {
  ...
  const fetchCars = useCallback(async () => {
    try {
      const res = await api.get('/api/vehicle?limit=100');
  ...
  const filtedCars = useMemo(() => cars.filter(
  ...
  const handleDelete = async (identifier) => {
    ...
    try {
      ...
      await api.delete(`/api/vehicle/${car._id}`);
      toast.success('Car Delete.');
      fetchCars();
    } catch (err) {
      console.error(err);
      toast.error(err.respose?.data.message || 'Failed to Delete the car')
    }
  };
  ...
  const handleEditSubmit = async (payload) => {
    try {
      if (!editingCar._id) {
        await api.post('/api/vehicle', payload);
        toast.success('Car added')
      } else {
        await api.put(`/api/vehicle/${editingCar._id}`, payload);
      ...
      console.error(err);
      toast.error(err.respose?.data.message || 'Failed to save car')
    }
  }

  return (
    <div className="min-h-screen bg-gray-250 p-4 sm:p6">
      <div className='relative mb-8 pt-16 text-center'>
        <div className='absolute inset-x-0 top-0 flex justify-center'>
          <div className='h-1 w-20 bg-gradient-to-r from-gray-300 to-gray-500 rounded-full'></div>
        </div>
        <h1 className='text-4xl font-extrabold py-4 text-white sm:text-5xl mb-3 tracking-wide'>
          <span className='text-transparent bg-clip-text bg-gradient-to-r from-gray-400 to-gray-500'>Fleet Management</span>
        </h1>
        <p className='text-gray-500 max-w-2xl mx-auto'>Manage your entire fleet efficiently and effectively.</p>
        </div>
      <div className='bg-gray-500/50 backdrop-blur-sm rounded-2xl p-5 mb-6 border border-gray-800'>
        <div className='flex flex-col md:flex-row items-center justify-between agp-6'>
          <StartCard title="Total Cars" value={cars.length} icon={FaCar} />
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
        {filtedCars.map((car) =>(
          <CarCard key={car.id} car={car} onEdit={openEdit} onDelete={handleDelete}/>
        ))}
      </div>

      {filtedCars.length === 0 && (
        <NoCarsVeiw onResetFilter={() => setCategoryFilter('all')}/>
      )}
